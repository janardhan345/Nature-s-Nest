...
const FeatureCard = ({ title, description, icon, image }) => {
  return (
    <div className="group relative bg-[var(--soft-sand)] p-8 rounded-lg shadow-sm border border-[var(--soft-sand)] hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 hover:bg-[var(--beige)] overflow-hidden hover:text-xl">
      <div className="text-[var(--deep-green)] mb-4 w-16 h-16 mx-auto">
        {icon}
      </div>
      <h3 className="font-playfair text-2xl mb-3 text-center text-[var(--deep-green)]">
        {title}
      </h3>
      ...
      {/* Hover image preview */}
      {image && (
        <div className="mt-6 relative rounded-xl overflow-hidden max-h-0 opacity-0 translate-y-2 transition-all duration-500 ease-out group-hover:max-h-56 group-hover:opacity-100 group-hover:translate-y-0">
...
const NaturalInteriorsIcon = () => (
  <svg
    ...
    fill="currentColor"
    className="bi bi-house-check w-full h-full"
    viewBox="0 0 16 16"
  >
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
    ...
    >
      <path
        ...
        d="M10 9.255C7.606 7.958 5.08 5.715 3 2m8.616 2.419C9.58 3.084 7.097 3.642 6.069 5.666s-.211 4.747 1.824 6.083c1.842 1.209 5.874 2.459 9.107-1.004c-3.03-1.29-3.35-4.99-5.384-6.326"
      />
  ...
  const features = [
    {
      ...
      description:
        "Sustainable, locally sourced materials that blend with the environment while providing ultimate comfort.",
      icon: <NaturalInteriorsIcon />,
      image: "/images/features/natural-interiors.jpg",
    },
    {
      title: "Local Organic Food",
      description:
        "Farm-to-table cuisine featuring fresh ingredients from our gardens and local producers.",
      icon: <OrganicFoodIcon />,
      image: "/images/features/organic-food.jpg",
    },
    {
      title: "No Plastic Policy",
      description:
        "Our commitment to zero single-use plastics preserves the pristine environments we call home.",
      icon: <NoPlasticIcon />,
      image: "/images/features/no-plastic.jpg",
    },
  ];

  return (
    <section
      className="py-20 px-6 bg-[var(--mint-green)] bg-opacity-10"
      id="features"
    >
      <div className="container mx-auto">
        <h2 className="text-3xl md:text-4xl text-[var(--deep-green)] font-playfair text-center mb-16">
